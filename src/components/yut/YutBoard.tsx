...
// Static line & path points connecting board positions
const OUTER_LINE_POINTS = Array.from({ length: 20 }, (_, i) => {
  const c = getPositionCoord(i);
  return coordToSVG(c.x, c.y);
});

// Diagonal A: corner 0 → 21 → 22 → center(20) → 23 → 24 → corner 10
const DIAGONAL_1_POINTS = [0, 21, 22, 20, 23, 24, 10].map((pos) => {
  const c = getPositionCoord(pos);
  return coordToSVG(c.x, c.y);
});

// Diagonal B: corner 5 → 25 → 26 → center(20) → 27 → 28 → corner 15
const DIAGONAL_2_POINTS = [5, 25, 26, 20, 27, 28, 15].map((pos) => {
  const c = getPositionCoord(pos);
  return coordToSVG(c.x, c.y);
});

const OUTER_PATH = OUTER_LINE_POINTS.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.cx} ${p.cy}`).join(' ') + ' Z';

interface PieceRenderModel {
  pieceId: string;
  playerId: number;
  position: number;
  isCarried: boolean;
...
/**
 * Builds render model descriptors for all pieces across players (home and board).
 */
function getPieceRenderModels(players: Player[]): PieceRenderModel[] {
  const pieces: PieceRenderModel[] = [];

  for (const player of players) {
    for (const piece of player.pieces) {
      if (piece.position === -2) continue; // finished, don't render

      const isCarried = player.pieces.some((p) => p.carrying.includes(piece.id));
  ...
  return (
    <div className="relative w-full max-w-[600px] aspect-square mx-auto">
      ...
      >
        ...
        <path d={OUTER_PATH} fill="none" stroke="#5C3A1A" strokeWidth={2.5} strokeLinejoin="round" />

        {/* Diagonal lines */}
        <polyline
          points={DIAGONAL_1_POINTS.map((p) => `${p.cx},${p.cy}`).join(' ')}
          fill="none"
          stroke="#5C3A1A"
          strokeWidth={2.5}
        ...
        />
        <polyline
          points={DIAGONAL_2_POINTS.map((p) => `${p.cx},${p.cy}`).join(' ')}
          fill="none"
          stroke="#5C3A1A"
          strokeWidth={2.5}
          strokeLinejoin="round"
        ...
        {getPieceRenderModels(players).map((p) => {
