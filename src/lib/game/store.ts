...
export type CapturedPiece = { pieceId: string; playerId: number };
...
export function findUpdatedCarriedPieces(
  player: Player,
  mainPieceId: string,
  ...
  if (isFinish || targetPos < 0) return carried;
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  _completeMove: (pieceId, targetPos, targetPath, isFinish) => {
    ...
    const captured = findCapturedPieces(state.players, player.id, newPos, isFinish);
    const carried = findUpdatedCarriedPieces(player, mainPieceId, allCarried, newPos, isFinish);
