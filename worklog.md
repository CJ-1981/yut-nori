...
Task ID: 50
Agent: Main (Super Z)
Task: Add GitHub Pages deployment support.

Work Log:
- Verified the game is fully client-side (Zustand state + localStorage).
  The /api route returns static JSON; Prisma is imported by lib/db.ts
  but never used by the game code. Static export is feasible.

- Updated `src/app/api/route.ts`:
  * Added `export const dynamic = "force-static"` so the route is
    compatible with `output: "export"`. Without this, Next.js
    refuses to build for static export.

- Updated `next.config.ts`:
  * Reads `DEPLOY_TARGET` env var. When set to `gh-pages`:
      - output: "export" (instead of "standalone")
      - trailingSlash: true
      - images.unoptimized: true
      - basePath + assetPrefix = `/${repoName}` (derived from
        GITHUB_REPOSITORY env var, auto-set by GitHub Actions)
  * When not set (default): keeps the original standalone config
    with outputFileTracingIncludes for rapier WASM.
  * This lets a single codebase target both Node.js hosts
    (Vercel/Render/Railway) and GitHub Pages.

- Added `public/.nojekyll` (empty file):
  * Tells GitHub Pages to skip Jekyll processing.
  * Without this, Jekyll would silently drop the `_next/` folder
    because folder names starting with `_` are special in Jekyll.

- Updated `package.json` scripts:
  * `build:gh-pages`: copies WASM to public/, runs next build with
    DEPLOY_TARGET=gh-pages, creates .nojekyll in out/.
  * `serve:gh-pages`: locally serves the out/ folder for preview.

- Created `.github/workflows/deploy-gh-pages.yml`:
  * Triggers on push to main/master.
  * Sets up Node 20, runs npm ci, builds with DEPLOY_TARGET=gh-pages.
  * Uploads ./out as a Pages artifact, then deploys via
    actions/deploy-pages@v4.
  * Pages URL is reported back to the workflow summary.

- Created `GH-PAGES-DEPLOY.md` with full instructions:
  * Quick start (3 steps).
  * How it works (basePath, WASM, .nojekyll, 404.html).
  * Local testing recipe (mkdir + cp + npx serve).
  * Custom domain instructions.
  * Notes on limitations (no server-side code, no image optimizer).

Verification:
- `npm run build:gh-pages` with GITHUB_REPOSITORY=user/yut-nori
  produces out/ with:
    * index.html (11 KB) referencing /yut-nori/_next/static/chunks/...
    * _next/static/chunks/*.js (chunks exist at this path)
    * rapier_wasm3d_bg.wasm (1.44 MB, application/wasm)
    * 404.html (auto-generated by Next.js)
    * .nojekyll (empty file)
- Served the out/ folder from /yut-nori/ subpath with `npx serve`:
    * GET /yut-nori/                         -> 200 (11 KB HTML)
    * GET /yut-nori/rapier_wasm3d_bg.wasm    -> 200 (1.44 MB, application/wasm)
    * GET /yut-nori/_next/static/chunks/...  -> 200 (16 KB JS chunk)
- Default `npm run build` (standalone) still works for Node.js hosts.
- `npm run dev` still works (no env vars needed).

Stage Summary:
- Project can now be deployed to GitHub Pages with zero code changes.
- One workflow file (.github/workflows/deploy-gh-pages.yml) handles
  the entire CI/CD pipeline.
- Both deployment targets (Node.js standalone + GitHub Pages static)
